...
foreach($form_fields as $key => $f){
    ...
    if($f['type'] == 'hidden'){
        $form_fields[$key]['value'] = $sanitizer->int($input->post->$key);
    }
        ...
        if(!$u->getErrors()){
            ...
            foreach($page_fields as $pf){
                if($templates->get($template)->hasField($pf)){
                    if($pf == 'career_listing'){
                        $ref = $pages->get($form_fields[$pf]['value']);
                        $uploadpage->$pf = $ref->title;
                    }
                    else $uploadpage->$pf = $form_fields[$pf]['value'];
                } else {
            ...
            $body = 'CV submitted for listing: '.$uploadpage->career_listing;
            $to = 'receptionug@clarkson-group.com';
            
            $m = \ProcessWire\wireMail();
            $m->to($to)->from('cv@clarkson-group.com');
            $m->subject('CV submitted');
            $m->body($body);
            $m->attachment($uploadpage->files->filename);
            $m->send();
