...
if($input->post->action == 'send'){
    ...
    if(empty($errors)) {
            ...
            $body = $form_fields['first_name']['value'].' '.$form_fields['last_name']['value'].' from '.$form_fields['country']['value'].' has submitted a risk management quotation request on '.date("Y-m-d H:i:s").' regarding the following sectors '.str_replace(';',', ',$products).'. Additional information: '.$mesg;
            ...
            $m->subject('Risk management quotation');
            $m->body($body);
